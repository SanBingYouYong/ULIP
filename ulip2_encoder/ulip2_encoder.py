...
class OBJProcessor:
    ...
    def load_obj_to_pointcloud(self, obj_path: str) -> np.ndarray:
        ...
        Returns:
            Point cloud as numpy array of shape (N, 3) or (N, 6) if colors available
        """
        try:
            ...
            if len(mesh.vertex_colors) > 0:
                colors = np.asarray(point_cloud.colors) if len(point_cloud.colors) > 0 else np.asarray(mesh.vertex_colors)[:len(points)]
